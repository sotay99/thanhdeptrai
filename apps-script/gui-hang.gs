    ...
    danhSach.forEach(function (don) {
      try {
        ...
        if (!don.email) {
          MailApp.sendEmail({
            to: emailShop,
            subject: '[Gửi tay] Đơn ' + don.__ma + ' — khách không để lại email',
            htmlBody: soanThuBaoShop(don, 'Khách KHÔNG để lại email. Vui lòng gửi tay qua Zalo hoặc SMS.')
          });
          capNhatDon(don.__ma, {
            trangThai: 'canXemTay',
            guiLuc: Date.now(),
        ...
        MailApp.sendEmail({
          to: emailShop,
          subject: (thu.thieu.length ? '[THIẾU LINK] ' : '[Đã gửi] ') + 'Đơn ' + don.__ma,
          htmlBody: soanThuBaoShop(don, thu.thieu.length
        ...
        try {
          ...
          MailApp.sendEmail({
            to: emailShop,
            subject: '[LỖI] Đơn ' + don.__ma + ' chưa gửi được',
            htmlBody: soanThuBaoShop(don, 'LỖI khi gửi: ' + loi.message)
          });
        } catch (loiNua) {
...
function kiemTraThietLap() {
  ...
  MailApp.sendEmail({
    to: emailShop,
    subject: '[Kiểm tra] Thiết lập gửi hàng tự động',
    htmlBody:
